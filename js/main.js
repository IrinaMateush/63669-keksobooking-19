...
var pinListElement = document.querySelector('.map__pins'); //сюда будем вставлять пины
var pinTemplate = document.querySelector('#pin').content.querySelector('.map__pin'); //шаблон пина

function getRandomInt(min, max) { //Максимум и минимум включительно
  min = Math.ceil(min);
  max = Math.floor(max);
...
function getArrayRandomLength(arr) {
  ...
  for (var i = arr.length - 1; i > 0; i--) { //перемешиваем массив по https://learn.javascript.ru/task/shuffle
...
//функция создания массива из x сгенерированных JS объектов заданной структуры
